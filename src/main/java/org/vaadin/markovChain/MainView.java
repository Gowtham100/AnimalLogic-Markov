package org.vaadin.markovChain;
...
public class MainView extends VerticalLayout {
    ...
    public MainView(@Autowired MarkovTextGenerator markov) {
        ...
        IntegerField prefixPoolSizeInput = new IntegerField ("Size of options");
        prefixPoolSizeInput.setValue(1);
        ...
        Button submitButton = new Button("Run Markov's Magic",
                e -> {
            if (prefixPoolSizeInput.getValue() > outputSizeInput.getValue()) {
                Notification.show("Uh Oh!! pool size can't be larger than the output value!!");
                UI.getCurrent().getPage().reload();
            } else {
                try {
                    String markovGen = markov.generateTextWithMarkov(outputValue.get(),
                            prefixPoolSizeInput.getValue(),
                            outputSizeInput.getValue());
                    Notification.show(markovGen);
                    // Download file
        ...
        add(uploadText, singleFileUpload, poolSizeText, prefixPoolSizeInput,outputSizeText,outputSizeInput, submitButton, resetButton);
    }
}